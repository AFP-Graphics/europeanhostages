...
function colorBy(criteria) {
  ...
  switch (criteria) {
    case 'locations':
      chart.selectAll("g.barGroup>rect")
...
function filterPictos(e) {
  ...
  if (filterByNationality(e) == true && (search == null || matcher.test(e.name) == true || matcher.test(normalize(e.name)) == true || matcher.test(e.profession_activity) == true || matcher.test(e.profession_activity) == true)) {
    return true;
  }
  var ok;
  //TODO : fix
  ...
  geojson.features.forEach(function(hevent,key) {
    if(Chart.data_indexed[hevent.properties.id]!==undefined) {
      ...
      geojson.features[key].properties.end = Chart.data_indexed[hevent.properties.id].date_hostage_status_ends;
        if(geojson.features[key].properties.end == null) {
  ...
  var timeline = L.timeline(geojson, {
    ...
    pointToLayer: function(geojson, latlng) {
      return L.circleMarker(latlng, {
        radius: Math.sqrt(Chart.data_indexed[geojson.properties
              .id].concerns_these_hostages.length *
            Math.PI) *
          3,
        color: "#fff",
        fillColor: "#0096c7",
...
$('nav li a').on('click', function() {
  $(this).tab('show');
  Chart.sortBy(this.dataset.tabtarget);
...
$('#moreInfo').on('show.bs.modal', function(e) {
  if (e.relatedTarget !== undefined) {
    var recipient = e.relatedTarget.dataset.eventid;
